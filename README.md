...
- **Постобработка** — нормализация, коррекция OCR и валидация по форматам стран
...
flowchart LR
    ...
    F -->|Достигнут кворум| G[Постобработка и валидация]
    F -->|Недостаточно данных| Z
    G -->|Валидный номер| H[Проверка кулдауна]
    G -->|Отброшено валидатором| Z
    H -->|Кулдаун прошёл| I[Сохранение в БД]
    H -->|В кулдауне| Z
    I --> J[Обновление UI]
```
...
### Валидация номеров
- **Плагины стран** — правила и коррекции описаны в YAML (`anpr/validation/configs`)
- **Поддержка кириллицы/латиницы** — нормализация символов по странам
- **Частые ошибки OCR** — замены похожих цифр/букв и удаление лишних символов
- **Настройка стран** — список кодов и стоп-слов задаётся в общих настройках приложения
...
- **Схема таблицы**:
  ...
  - `plate` — распознанный номер после валидации
  - `raw_text` — исходное значение из OCR перед постобработкой
  - `country_code` / `country_name` — определённая страна
  - `plate_format` — совпавший шаблон
  - `validation_reason` — причина отклонения (если была)
