...
- **Валидация номеров** — постобработка по шаблонам стран (RU/BY/KZ), коррекция OCR-ошибок и отсев тестовых значений
...
## 🛡️ Валидация и постобработка

- **Этап после агрегации треков** — валидация вызывается только для консенсусного номера, что исключает лишние вычисления на бестшотах.
- **Плагины стран** — YAML-конфигурации в `anpr/validation/configs` (RU/BY/KZ) с приоритетами и регулярными выражениями форматов.
- **Коррекция OCR** — замена похожих символов (0→О, 1→I/Т, 8→В/B и др.), латиница↔кириллица для соответствующих стран.
- **Фильтры качества** — стоп-слова (`TEST`, `SAMPLE`), монотонные последовательности, проверка длины и допустимых символов.
- **Гибкие настройки** — выбрать список стран и стоп-слов можно в секции `validation` общих настроек (`settings.json`).
...
flowchart LR
    ...
    F -->|Достигнут кворум| V[Валидация и коррекция]
    F -->|Недостаточно данных| Z
    V -->|Валиден| G[Проверка кулдауна]
    V -->|Отброшен| Z
...
- **Схема таблицы**:
  ...
  - `plate` — финальный валидированный номер
  - `raw_plate` — строка до постобработки
  - `country` / `country_name` — код и название шаблона страны
  - `plate_format` — совпавший тип номера
  - `channel` — человекочитаемое имя канала
  - `frame_path` — путь к полному кадру
  - `plate_path` — путь к кропу номера
