...
class EventDatabase:
    ...
    def _ensure_columns(conn: sqlite3.Connection) -> None:
        ...
        if not _column_exists("raw_plate"):
            conn.execute("ALTER TABLE events ADD COLUMN raw_plate TEXT")
    ...
    def insert_event(
        ...
        with self._connect() as conn:
            cursor = conn.execute(
                (
                    "INSERT INTO events (timestamp, channel, plate, confidence, source, frame_path, plate_path, country, country_name, plate_format, raw_plate)"
                    " VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
                ),
                (
                    ts,
...
class AsyncEventDatabase:
    ...
    async def _ensure_columns(self, conn: aiosqlite.Connection) -> None:
        ...
        if not await _column_exists("raw_plate"):
            await conn.execute("ALTER TABLE events ADD COLUMN raw_plate TEXT")
